...
if __name__ == "__main__":
  if not isLocked(session=session, table=settings_table):
    ...
    lisence_key = input("Enter Lisence Key: ")
    lisences = readLisences(conf.LISENCES_PATH)
    result = matchLisence(lisences=lisences, my_lisence=lisence_key)
    if result:
      removeLisence(path=conf.LISENCES_PATH, lisences=lisences, my_lisence="abcd")
